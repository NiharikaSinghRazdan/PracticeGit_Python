# Use for, .split(), and if to create a Statement that will print out words that start with 's':
st = 'Print only the words that start with s in this sentence'
my_list=st.split()
print(my_list)
for item in my_list:
    if item[0]=='s':
        print(item)    

# Use range() to print all the even numbers from 0 to 10.

for num in range(0,11):
    if num%2==0:
        print(num)

# Use List comprehension to create a list of all numbers between 1 and 50 that are divisible by 3.
my_list1=[]
mylist1=[num for num in range(1,51) if num%3==0]
print(mylist1)

# Go through the string below and if the length of a word is even print "even!"
st = 'Print every word in this sentence that has an even number of letters'
mylist=[]
mylist=st.split()
print(mylist)
for item in mylist:
    if len(item)%2==0:
        print("even for item {}".format(item))

#Write a program that prints the integers from 1 to 100. But for multiples of three print "Fizz" instead of 
#the number, and for the multiples of five print "Buzz". For numbers which are multiples of both three and five
# print "FizzBuzz".

for num in range(1,100):
    if (num%3==0 and num%5==0):
        print("FizzBuzz for number{}".format(num))
    elif num%5==0:
        print("Buzz for number{}".format(num))
    elif(num%3==0):
        print("Fizz for number {}".format(num))

#Use a List Comprehension to create a list of the first letters of every word in the string below:
st = 'Create a list of the first letters of every word in this string'
mylist=[]
mylist1=[]
mylist=st.split()
mylist1=[char[0] for char in mylist]
print(mylist1)